...
$("body").on("click", "INSERT SUBMIT BUTTON", function () {
    e.preventDefault();
    var search = $("INSERT INPUT VALUE").val().trim();
    console.log(search);
    var queryURL = "https://api.giphy.com/v1/gifs/search?q=" + search + "&api_key=PXKd89OxBUGSE1Ptn1uQAXeN3hNlg5uc&limit=10";
    ...
    $.ajax({
        ...
        for (var j = 0; j < response.data.length; j++) {
            ...
            $("INSERT ID FROM RELAX PAGE").prepend(gifDiv);
        }
    ...
    e.preventDefault();
        ...
        request.execute(function (response) {
            ...
            $("INSERT ID FROM HTML FOR DIV WITH VIDEOS").html("");
            $.each(results.items, function (index, item) {
                $.get("tpl/item.html", function (data) {
                    $("INSERT ID FROM HTML FOR DIV WITH VIDEOS").append(tplawesome(data, [{ "title": item.snippet.title, "videoid": item.id.videoId }]));
                });
            });
            resetVideoHeight();
        });
